...
class Encoder_TRANSFORMER(nn.Module):
    ...
    def forward(self, batch):
        ...
        # x.shape为(n_frame,batch_size,njoints*nfeats)
        ...
        if self.ablation == "average_encoder":
            ...
            # xseq.shape为(2+n_frame,batch_size,latent_dim)
            xseq = torch.cat((self.muQuery[y][None], self.sigmaQuery[y][None], x), axis=0)
            #print(xseq.shape)
            # add positional encoding
            # positionalencoder后xseq.shape仍然为(2+n_frame,batch_size,latent_dim)
...
class Decoder_TRANSFORMER(nn.Module):
    ...
    def forward(self, batch):
        ...
        if self.ablation == "time_encoding":
            ...
            # decoder只对1，2，3，。。。做positional encoding
