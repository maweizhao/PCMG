...
SMPL_MODEL_PATH="./lib/smpl/basicmodel_f_lbs_10_207_0_v1.1.0.pkl"
...
def rot2xyz(args,x,betas,mask=None):
    ...
    rotation2xyz = Rotation2xyz(device=torch.device("cuda:0"),model_path=SMPL_MODEL_PATH)
...
def main():
    ...
    if args.mocap_marker:
        from _dataset.src.mocap_marker import finetune_PICK_INDEX as PICK_INDEX,finetune_PICK_NORMAL,distance_from_skin
    ...
    for cls in range(args.num_cls):
        ...
        for index,pose in tqdm(enumerate(data['poses']),total=len(data['poses'])):
            ...
            #pose=torch.zeros([lengths,jointnum_mul_3])
            ...
            offset=finetune_PICK_NORMAL.unsqueeze(0).repeat(args.num_frames,1,1)*distance_from_skin
            vertices=vertices+offset.cuda()
            
            # print(vertices.shape)
            #meshplot_visuals_n_seq_color([-vertices],["red"])
            #break
            #from _dataset.src.process_c3d import write_mocap_c3d
            # points=vertices.cpu().numpy()*1000
            # points=points
            # print(points.shape)
            # write_mocap_c3d(points,"random_001_60frame.c3d",frame_rate=10)
