...
def hybrik(betas, global_orient, pose_skeleton, phis,
    ...
    device = betas.device
    shapedirs=shapedirs.to(device)
    J_regressor=J_regressor.to(device)
    #print(device)

    # 1. Add shape contribution
    v_shaped = v_template.to(device) + blend_shapes(betas, shapedirs)

    # 2. Get the rest joints
    # NxJx3 array
    if leaf_thetas is not None:
        rest_J = vertices2joints(J_regressor.to(device), v_shaped)
    else:
        ...
        #parents=torch.cat([parents,torch.tensor([15,22,23,10,11])],dim=0)
    ...
    ident = torch.eye(3, dtype=dtype, device=device)
    pose_feature = (rot_mats[:, 1:] - ident).view([batch_size, -1]).to(device)
    pose_offsets = torch.matmul(pose_feature, posedirs.to(device)) \
        .view(batch_size, -1, 3)
    ...
    # 5. Do skinning:
    # W is N x V x (J + 1)
    W = lbs_weights.unsqueeze(dim=0).expand([batch_size, -1, -1]).to(device)
    # (N x V x (J + 1)) x (N x (J + 1) x 16)
    num_joints = J_regressor.shape[0]
    T = torch.matmul(W, A.view(batch_size, num_joints, 16)) \
        .view(batch_size, -1, 4, 4)
    ...
    verts = v_homo[:, :, :3, 0]
    J_from_verts_h36m = vertices2joints(J_regressor_h36m.to(device), verts)
